  ...
  render() {
    return (
      <div>
        ...
        <ApiKeyConfigurator
          configureApiKey={this.configureApiKey}
      ...
      <div className="row">
        ...
        >
          ...
          <button
            ...
            style={{ marginLeft: 10 }}
            onClick={() => this.props.configureApiKey(this.state.apiKey)}
          >
            configure
          </button>
        </div>
  ...
  render() {
    return (
      <div>
        ...
        <button
          type="button"
          onClick={e => this.props.handler(this.state.text)}
        >
          Submit
        </button>
      </div>
...
class ItemsList extends React.Component {
  constructor(props) {
    ...
    this.state = { items: [] };
    getItems(
      `https://6lorfb62u4.execute-api.eu-west-1.amazonaws.com/foo`,
      props.apiKey
...
const Item = ({ item }) => (
  <li>
    {item.text}
    <i
      onClick={e => playSound(e, item.mp3_location)}
      className="fas fa-volume-up"
      style={{ display: item.status === "PROCESSED" ? "" : "none" }}
    />
  </li>
);
